...
kilns_AB <- kilns_AB %>% 
  plyr::mutate(LOTNO = as.factor(LOTNO),
               ...
               date = as.Date(date),
               time = new_time) %>% 
  dplyr::select(c(LOTNO, kiln, date, time,
                  kilntemp, temp_setpoint, 
                  output, kilnpress, pres_wsp, diffair, incin_temp))

# data.frame(names(kilns_AB))
names <- names(kilns_AB)
names[c(5,6)] <- c("avg_kiln_temp", "setpoint")
names(kilns_AB) <- names
...
kilns_C <- kilns_C %>% 
  plyr::mutate(LOTNO = as.factor(LOTNO),
               ...
               time = new_time) %>% 
  dplyr::select(c(LOTNO, kiln, date, time,
                  avg_kiln_temp, temp_sp, 
                  t_c_51a, t_c_51b, t_c_52a, t_c_52b, 
                  everything())) %>% 
  dplyr::select(-c(start_temp, diff_setpoint, max_diff_setpoint, temp_slope, diff_start, max_temp, new_time))
  
names <- names(kilns_C)
names[c(6)] <- c("setpoint")
names(kilns_C) <- names
...
#   # dplyr::filter(LOTNO %in% levels(kilns_C$LOTNO)[1:6]) %>%
...
#   geom_point(size = .8)+
#   theme(legend.position = "none")+
#   facet_wrap(~LOTNO, scales = "free_y")
...
kilns_D <- kilns_D %>% 
  plyr::mutate(LOTNO = as.factor(LOTNO),
               ...
               time = new_time) %>% 
  dplyr::select(c(LOTNO, kiln, date, time,
                  avg_kiln_temp, setpoint, 
                  bot_t_c_1, bot_t_c_2, top_t_c_3, top_t_c_4, 
                  everything())) %>% 
  dplyr::select(-c(start_temp, diff_setpoint, max_diff_setpoint, temp_slope, diff_start, new_time, max_temp))
# names(kilns_D)
...
#   group_by(LOTNO) %>%
#   ggplot(aes(x=time, y = setpoint ,color=LOTNO))+
#   geom_point(size = .8)+
...
for(kiln in kilns[c(6)]){
  ...
  for(sub_dir in list.dirs(kiln)[2:length(list.dirs(kiln))]){
    ...
    for(i in seq(max_temp_index, nrow(df))){
      ...
      if( (slope > 1) & (diff_start > -250) ){
        temp_list <- append(temp_list, i)
      }
    }
    
    # if(!is.na(temp_list)){splice_end <- temp_list[[1]]}
    if(length(temp_list) > 0 ){splice_end <- temp_list[[1]]}
    else{splice_end <- nrow(df)}
    # else{splice_end <- temp_list[[1]]}
...
#   plyr::mutate(LOTNO = as.factor(LOTNO))
# kilns_F %>%
#   dplyr::filter(LOTNO %in% levels(kilns_F$LOTNO)[1:20]) %>%
#   # dplyr::filter(LOTNO %in% levels(kilns_F$LOTNO)[21:40]) %>%
#   # dplyr::filter(LOTNO %in% levels(kilns_F$LOTNO)[41:60]) %>%
...
kilns_G <- kilns_G %>%
  plyr::mutate(LOTNO = as.factor(LOTNO),
               ...
               time = new_time,
               setpoint_top = tmprsp,
               setpoint_bot = bzone_sp
               ) %>% 
  dplyr::select(c(LOTNO, kiln, date, time,
                  avg_kiln_temp, setpoint,
                  avg_top_kiln_temp, setpoint_top, 
                  tc_t1, tc_t2, tc_t3, tc_t4, 
                  avg_bot_kiln_temp, setpoint_bot,
                  tc_b1, tc_b2, tc_b3, tc_b4, 
                  everything())) %>% 
  dplyr::select(-c(start_temp, diff_setpoint, max_diff_setpoint, temp_slope, diff_start, new_time, max_temp, tmprsp, bzone_sp))
# # names(kilns_G)
# 
...
kilns_H <- kilns_H %>%
  plyr::mutate(LOTNO = as.factor(LOTNO),
               ...
               date = as.Date(date),
               time = new_time,
               setpoint_top = setpoint.x,
               setpoint_bot = setpoint.y
  ) %>%
  dplyr::select(c(LOTNO, kiln, date, time,
                  avg_kiln_temp, setpoint,
                  avg_top_kiln_temp, setpoint_top,
                  t_c_1t, t_c_2t, t_c_3t, t_c_4t, 
                  avg_bot_kiln_temp, setpoint_bot,
                  t_c_1b, t_c_2b, t_c_3b, t_c_4b, 
                  everything())) %>%
  dplyr::select(-c(start_temp, diff_setpoint, max_diff_setpoint, temp_slope, diff_start, new_time, max_temp, setpoint.x, setpoint.y))
# # names(kilns_H)
# 
...
#   # dplyr::filter(LOTNO %in% levels(kilns_H$LOTNO)[1:10]) %>%
